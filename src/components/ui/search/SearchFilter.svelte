...
>
	<h3>Filter By Tag</h3>
	<button class="filter" on:click={() => ($filter = "all")} class:off={$filter !== "all"}>
		<Icon icon="filter" />
		All
	</button>
	<button class="filter" on:click={() => ($filter = "favorite")} class:off={$filter !== "favorite"}>
		<Icon icon="star" color="var(--color-favorite-light)" />
		Favorite
	</button>
	<button class="filter" on:click={() => ($filter = "mission")} class:off={$filter !== "mission"}>
		<Icon icon="exclamation-circle" color="var(--color-mission)" />
		Mission Requirement
	</button>
	<button class="filter" on:click={() => ($filter = "fortress")} class:off={$filter !== "fortress"}>
		<Icon icon="fortress" />
		Fortress
	</button>
