// Read the README.md inside the data/translations folder to learn more about this file.
export const keys = {
	"ahmed-grocery-store": {
		title: "アフメドの食料品店",
		keyName: "アフメドの食料品店の鍵",
	},
	"airport-maintenance": {
		title: "空港のメンテナンス室",
	},
	"abf-antiquities-office": {
		title: "A.B.F.古物商の事務所"
	},
	"al-bagra-barrack": {
		title: "アル・バグラの兵舎",
	},
	"al-bagra-munitions-storeroom": {
		title: "アル・バグラの軍需物資保管室",
	},
	"al-bagra-officers-quarters": {
		title: "アル・バグラの士官棟",
	},
	"al-bagra-underground": {
		title: "アル・バグラの地下",
	},
	"algae-covered-toolbox": {
		title: "藻類に覆われたツールボックス",
	},
	"al-said-shopping-centre": {
		title: "アル・サイードのショッピングセンター",
	},
	"al-sharim-pass-elders-room": {
		title: "アル・シャリム・パスの老人の部屋",
	},
	"al-safwa-stone-block-office": {
		title: "アル・サファの石造りのオフィス"
	},
	"art-museum": {
		title: "美術館",
	},
	"bc-toolbox": {
		title: "B.C.ツールボックス",
	},
	"bank-of-adal-top-story-office": {
		title: "アダル銀行の最上階オフィス",
	},
	"bank-of-adal-2nd-story-office": {
		title: "アダル銀行の2階オフィス",
	},
	"canal-apartment-103": {
		title: "運河にあるアパート 103号室",
	},
	"captains-cache": {
		title: "船長の貯蔵庫",
	},
	"caretakers-house": {
		title: "管理人の家",
	},
	"caretakers-toolbox": {
		title: "管理人のツールボックス",
	},
	"cavern-boat-dock-shack": {
		title: "洞窟のボートドック小屋",
	},
	"central-said-top-floor-apartment": {
		title: "サイード中心部の最上階アパート",
	},
	"central-zaya-meeting-room": {
		title: "ザヤ中心部の会議室",
	},
	"chemical-storage-warehouse": {
		title: "薬品貯蔵倉庫",
	},
	"ch-7-editorial-department": {
		title: "Ch 7 の編集部",
	},
	"ch-7-secure-records-room": {
		title: "Ch 7 の警備記録室",
	},
	"clock-repair-tools": {
		title: "時計修理道具",
	},
	"control-tower": {
		title: "管制塔",
	},
	"crane-control-room": {
		title: "クレーン制御室",
	},
	"deckhands-toolbox": {
		title: "甲板員のツールボックス",
	},
	"downtown-post-office": {
		title: "市街地の郵便局",
	},
	"downtown-post-office-secure-room": {
		title: "市街地にある郵便局の警備室",
	},
	"enfer-back-room": {
		title: "エンファーのバックルーム",
	},
	"far-south-eastern-storage-closet": {
		title: "南東の外れにある保管庫のクローゼット",
	},
	"groundskeepers-building": {
		title: "グラウンド管理人室",
	},
	"hafid-ship-bridge-cache": {
		title: "ハフィドにある船のブリッジのコンテナ",
	},
	"hafid-ship-first-mate-cache": {
		title: "ハフィドにある船の一等航海士のコンテナ",
	},
	"zarqwa-hydro-island-computer": {
		title: "ハイドロ島にあるコンピューター",
	},
	"west-zarqwa-safe-house": {
		title: "ザルクワ西部のセーフハウス",
	},
	"ihtaq-warehouse-supply-room": {
		title: "IHTAQ倉庫の備品室",
	},
	"kushaak-construction-warehouse": {
		title: "クシャークの建設倉庫",
	},
	"longshoremans-duffel-bag": {
		title: "湾岸労働者のダッフルバッグ",
	},
	"mawizeh-cell-shop": {
		title: "マウィゼにある携帯電話ショップ",
	},
	"mawizeh-power-control-substation": {
		title: "マウィゼの電源管理所の変電室",
	},
	"mawizeh-resort-bungalow-room": {
		title: "マウィゼにあるリゾートのバンガローの部屋",
	},
	"mawizeh-resort-main-building": {
		title: "マウィゼにあるリゾートのメインビル",
	},
	"mawizah-slum-discarded-cache": {
		title: "マウィゼのスラム街にある廃コンテナ",
	},
	"mountaintop-spotter-shack": {
		title: "山頂のスポッター小屋",
	},
	"north-canals-info-booth": {
		title: "運河北部にある案内所",
	},
	"old-lighthouse-citadel-room": {
		title: "古い灯台の砦の部屋",
	},
	"omars-auto-repair": {
		title: "オマーの自動車修理工場",
	},
	"police-academy": {
		title: "警察学校",
	},
	"police-armory-f2": {
		title: "警察署の武器庫 [F2]",
		keyName: "アル・マズラ警察署の武器庫の鍵",
	},
	"police-locker-f2": {
		title: "警察署のロッカー [F2]",
		keyName: "警察署のロッカーのマスターキー",
	},
	"police-armory-g5": {
		title: "警察署の武器庫 [G5]",
		keyName: "アル・マズラ警察署の武器庫の鍵",
	},
	"police-locker-g5": {
		title: "警察署のロッカー [G5]",
		keyName: "警察署のロッカーのマスターキー",
	},
	"police-armory-d6": {
		title: "警察署の武器庫 [D6]",
		keyName: "アル・マズラ警察署の武器庫の鍵",
	},
	"police-locker-d6": {
		title: "警察署のロッカー [D6]",
		keyName: "警察署のロッカーのマスターキー",
	},
	"police-academy-private-locker-room": {
		title: "警察学校のプライベートロッカー室",
	},
	"police-locker-g4": {
		title: "警察署のロッカー [G4]",
		keyName: "警察署のロッカーのマスターキー",
	},
	"police-academy-server-admin": {
		title: "警察学校のサーバー管理室",
	},
	"power-substation-toolbox": {
		title: "変電所のツールボックス",
	},
	"quarry-workers-lost-toolbox": {
		title: "採石場労働者の使われていないツールボックス",
	},
	"special-ops-relay-station": {
		title: "特殊部隊のリレーステーション",
	},
	"retro-runway-fridge": {
		title: "滑走路のレトロな冷蔵庫",
	},
	"rohan-control-room-locker": {
		title: "ロハンの制御室ロッカー",
	},
	"rohan-oil-north-guard-shack": {
		title: "ロハンオイルの北警備室",
	},
	"rohan-oil-south-guard-shack": {
		title: "ロハンオイルの南警備室",
	},
	"rohan-utility-tunnel-locker": {
		title: "ロハンのユーティリティトンネルのロッカー",
	},
	"said-mall-security-room": {
		title: "サイードにあるモールのセキュリティルーム",
	},
	"sattiq-guest-quarters": {
		title: "サティークにある宿泊所"
	},
	"sattiq-poppy-farm-house": {
		title: "サティークにあるケシの農家住宅",
	},
	"sawah-crypto-mining-farm-key": {
		title: "サワーの暗号通貨マイニングファーム",
	},
	"sawah-hotel-room-302": {
		title: "サワーにあるホテルの302号室",
	},
	"sawah-hotel-room-303": {
		title: "サワーにあるホテルの303号室",
	},
	"scientists-locker": {
		title: "科学者のロッカー",
	},
	"south-al-bagra-top-floor-bedroom": {
		title: "南アル・バグラの最上階ベッドルーム",
	},
	"south-bank-apartment": {
		title: "南岸のアパート",
	},
	"south-zaya-scientist-apartment": {
		title: "南ザヤの科学者のアパート",
	},
	"special-forces-dead-drop": {
		title: "特殊部隊のデッド・ドロップ",
	},
	"taraq-river-supply-shack": {
		title: "タラク川の補給小屋",
	},
	"taraq-smugglers-office": {
		title: "タラクの密輸人のオフィス",
	},
	"taraq-tunnels-workbench-cache": {
		title: "タラクのトンネル作業台にあるコンテナ",
	},
	"tower-conference-room": {
		title: "塔の会議室",
	},
	"travellers-luggage": {
		title: "旅行者の荷物",
	},
	"under-freeway-east-warehouse": {
		title: "高速道路下の東倉庫",
	},
	"us-embassy": {
		title: "アメリカ大使館",
	},
	"weapons-repair-kit": {
		title: "武器修理キット",
	},
	"yum-yum-burger-back-room": {
		title: "Yum Yumバーガーのバックルーム",
	},
	"zarqwa-market-storage": {
		title: "ザルクワのマーケット倉庫",
	},
	"zarqwa-red-village-appartment": {
		title: "ザルクワの村の赤いアパート",
	},
	"zaya-radar-dome": {
		title: "ザヤのレーダードーム",
	},
	"sunken-ship-thiefs-cache": {
		title: "泥棒のコンテナ",
	},
	"control-room": {},
	"fort-barrack": {},
	storage: {},
	"foremans-office": {},
	"stronghold-interior": {},
	"the-architects-toolbox": {
		title: "建築家のツールボックス",
	},
	"crash-site-weapon-case-under-pipeline": {
		title: "墜落現場の武器ケース [Under Pipeline]",
		keyName: "墜落現場の武器ケースの鍵",
	},
	"crash-site-weapon-case-airplane": {
		title: "墜落現場の武器ケース [Airplane]",
		keyName: "墜落現場の武器ケースの鍵",
	},
	"sc-special-forces-footlocker": {
		title: "SCのトランク",
	},
	"captain-silvers-briefcase": {
		title: "シルバー大尉のブリーフケース",
	},
	"the-lieutenants-briefcase": {
		title: "中尉のブリーフケース",
	},
	"bomb-makers-supply-cache": {
		title: "ボムメーカーのサプライコンテナ",
	},
		"b21-secure-cache":	{
		title: "B21のセキュアコンテナ",
	},
	"tsuki-castle": {
		title: "ツキ・キャッスル",
		keyName: "ツキ・キャッスル隠れ家の鍵",
	},
	"tsuki-castle-weapons-locker": {
		title: "ツキ・キャッスル武器ロッカー",
	},
	"lost-managers-office": {
		title: "使われていないマネージャー事務所",
	},
	"research-center-room": {
		title: "リサーチセンター室",
	},
	"water-pump-control": {
		title: "水ポンプ管理室",
	},
	"city-hall": {
		title: "シティホール",
		keyName: "シティホール隠れ家の鍵",
	},
	"hms-shipwreck-cache": {
		title: "H.M.S.難破船コンテナ",
	},
	"lost-room-403": {
		title: "使われていない403号室",
	},
	"main-harbor-control": {
		title: "主要港湾管理室",
	},
	"ashika-island-information-booth": {
		title: "アシカアイランド案内所",
	},
	"beach-club-bathhouse": {
		title: "ビーチクラブ浴場",
		keyName: "浴場係員の鍵",
	},
	"farmers-lunchbox": {
		title: "農家の弁当箱",
	},
	"floatsam-cargo-cache": {
		title: "フロートサム貨物コンテナ",
	},
	"mud-covered-cache": {
		title: "泥まみれのコンテナ",
	},
	"rusted-fridge": {
		title: "古びた冷蔵庫",
	},
	"waterlogged-bag": {
		title: "水浸しのバッグ",
	},
	"combat-engineer-toolkit": {
		title: "工兵の道具箱",
	},
	"drifting-supply-bag": {
		title: "漂着したサプライバッグ",
	},
	"waterway-maintenance-kit": {
		title: "水路メンテナンスキット",
	},
	"hotel-employee-fridge": {
		title: "ホテル従業員冷蔵庫",
	},
};
